...
- **Qwen Model Built-in Tools**: [`.agents/rules/qwen-model-builtin-tools.md`](.agents/rules/qwen-model-builtin-tools.md) — reference for the built-in Harness tools of Token-Plan Qwen models: the five tools (web search, code interpreter, web scraping, reverse image search, text-to-image search), the per-model support matrix (qwen3.8-max/flash and qwen3.7-plus support all five; qwen3.7-max lacks the image-search tools), and per-successful-invocation credit billing. Token Plan only — not Coding Plan; tools invoke automatically with no client configuration.
